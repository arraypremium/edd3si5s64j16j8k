...
class AnalyticsService
{
    ...
    /**
     * New member signups grouped by date.
     *
     * @return array<string, int>
     */
    public function newMemberTrendByDate(AnalyticsDateRange $range): array
    {
        ...
        $rows = Member::query()
            ...
            ->orderBy('day')
            ->pluck('total', 'day')
            ->map(fn ($value): int => Data::int($value));
    ...
    /**
     * New member signups grouped by month.
     *
     * @return array<string, int>
     */
    public function newMemberTrendByMonth(AnalyticsDateRange $range): array
    {
        $driver = Member::query()->getModel()->getConnection()->getDriverName();
        $monthExpression = $this->monthGroupExpression('created_at', $driver);
    ...
    /**
     * Renewals grouped by date.
     *
     * @return array<string, int>
     */
    public function renewalTrendByDate(AnalyticsDateRange $range): array
    {
        /** @var Collection<string, int> $rows */
    ...
    /**
     * Renewals grouped by month.
     *
     * @return array<string, int>
     */
    public function renewalTrendByMonth(AnalyticsDateRange $range): array
    {
        $driver = Subscription::query()->getModel()->getConnection()->getDriverName();
        $monthExpression = $this->monthGroupExpression('start_date', $driver);
    ...
    /**
     * Expired memberships without renewals grouped by date.
     *
     * @return array<string, int>
     */
    public function expiredTrendByDate(AnalyticsDateRange $range): array
    {
        /** @var Collection<string, int> $rows */
        $rows = Subscription::query()
            ->whereBetween('end_date', [$range->start->toDateString(), $range->end->toDateString()])
            ->whereDoesntHave('renewals')
            ->selectRaw('end_date as day')
            ->selectRaw('COUNT(*) as total')
            ->groupBy('day')
            ->orderBy('day')
            ->pluck('total', 'day')
    ...
    /**
     * Expired memberships without renewals grouped by month.
     *
     * @return array<string, int>
     */
    public function expiredTrendByMonth(AnalyticsDateRange $range): array
    {
        $driver = Subscription::query()->getModel()->getConnection()->getDriverName();
        $monthExpression = $this->monthGroupExpression('end_date', $driver);
