...
class SubscriptionForm
{
                ...
                Group::make()
                    ...
                    ->schema([
                        Select::make('member_id')
                            ...
                            ->required(),
                        Select::make('plan_id')
                            ->columnSpan(fn ($livewire): array => ($livewire instanceof SubscriptionsRelationManager ||
                            ...
                            ->afterStateUpdated(function (Get $get, Set $set) {
                                ...
                                // ─── FIX ──────────────────────────────────────────────────────
                                // Read the raw Livewire state directly instead of going through
                                // invoiceItems(), which was stripping Filament's internal UUID
                                // keys by using `$normalized[] = $item` (sequential re-index).
                                //
                                // That made $itemKey become "0", so $set("invoices.0.*") wrote
                                // to a brand-new integer-keyed slot while the original
                                // UUID-keyed item still existed.  The Repeater then rendered
                                // BOTH entries — one extra row per plan change.
                                //
                                // By iterating $get('invoices') directly we keep the real UUID
                                // key and $set() patches the existing item in-place.
                                // ──────────────────────────────────────────────────────────────
                                $rawInvoices = $get('invoices');

                                if (is_array($rawInvoices)) {
                                    foreach ($rawInvoices as $itemKey => $invoice) {
                                        if (! is_array($invoice)) {
                                            continue;
                                        }
                                        ...
                                        // $itemKey is the UUID Filament assigned — updating the
                                        // existing Repeater item in-place, never creating a new one.
    ...
    /**
     * Return the Repeater's current items, preserving Filament's internal UUID keys.
     *
     * IMPORTANT: the keys MUST be preserved.  The original implementation used
     * `$normalized[] = $item` which re-indexed with sequential integers (0, 1, 2…).
     * Any caller that then did $set("invoices.{$key}.*") would write to a *new*
     * integer-keyed slot instead of updating the existing UUID-keyed item, causing
     * the Repeater to render duplicate rows.
     *
    ...
    private static function invoiceItems(Get $get): array
    {
        ...
        foreach ($items as $key => $item) {            // FIX: capture $key
            if (! is_array($item)) {
                continue;
            }

            $normalized[$key] = \App\Support\Data::map($item);  // FIX: preserve UUID key
        }
