...
use App\Services\Backup\ApplicationBackupService;
use Carbon\Carbon;
...
class Settings extends Page implements HasForms
{
    ...
    /**
     * Header actions for full-site backup and restore.
     *
    ...
    {
        return [
            Action::make('download_full_backup')
                ->label('Download Full Backup')
                ->icon('heroicon-o-archive-box-arrow-down')
                ->color('success')
                ->action(fn () => app(ApplicationBackupService::class)->downloadBackupZip()),
            ...
            Action::make('import_full_backup')
                ...
                ->modalHeading('Import Full Backup')
                ->modalDescription('This restores the SQL backup and can replace existing application data. Use only on a fresh/new site or after taking a backup.')
                ->modalSubmitActionLabel('Import Backup')
                ->schema([
                    FileUpload::make('backup_file')
                        ->label('Backup .zip or .sql file')
                        ->disk('local')
                        ...
                        ->helperText('Upload the .zip generated by Download Full Backup, or a .sql backup file. SQL is used for import because it preserves IDs and relationships.')
                        ->maxSize(102400)
                        ->required(),
                ])
                ->action(function (array $data): void {
                    ...
                    if (! is_string($file) || $file === '') {
                        Notification::make()
                            ->title('Import failed')
                            ->body('Please upload a valid .zip or .sql backup file.')
                            ->danger()
                            ->send();
                    ...
                    if (! in_array($extension, ['zip', 'sql'], true)) {
                        ...
                        Notification::make()
                            ->title('Import failed')
                            ->body('Only .zip and .sql backup files are allowed.')
                            ->danger()
                            ->send();
                    ...
                    $result = app(ApplicationBackupService::class)->importBackup($path);

                    Storage::disk('local')->delete($file);

                    $notification = Notification::make()
                        ->title($result['success'] ? 'Backup imported' : 'Backup import failed')
                        ->body($result['message']);
