...
class MembershipOverviewSubscriptionsTableWidget extends TableWidget
{
    ...
    {
        $isAllowed = DashboardAccess::allowsWidget(DashboardAccess::WIDGET_MEMBERSHIP_OVERVIEW_TABLE);
        $title = $isAllowed
            ? __('app.navigation.groups.memberships')
            : DashboardAccess::lockedHeading(__('app.navigation.groups.memberships'));
...
BLADE,
    ...
    {
        if (! DashboardAccess::allowsWidget(DashboardAccess::WIDGET_MEMBERSHIP_OVERVIEW_TABLE)) {
                ...
                ActionGroup::make([
                    Action::make('renew')
                        ...
                        ->visible(function (Subscription $record) use ($today): bool {
                            if (! (auth()->user()?->can('Update:Subscription') ?? false)) {
                                return false;
                            }
                    ...
                    ViewAction::make()
                        ->visible(fn (): bool => auth()->user()?->can('View:Subscription') ?? false)
                        ->url(fn (Subscription $record): string => SubscriptionResource::getUrl('view', ['record' => $record])),
                ])
                    ->visible(fn (): bool => DashboardAccess::allowsWidget(DashboardAccess::WIDGET_MEMBERSHIP_OVERVIEW_TABLE)),
            ])
            ->emptyStateHeading(fn (): string|HtmlString => DashboardAccess::allowsWidget(DashboardAccess::WIDGET_MEMBERSHIP_OVERVIEW_TABLE)
                ? ($this->activeTab === 'expired'
                    ? __('app.widgets.no_expired_memberships')
                    : __('app.widgets.no_expiring_memberships'))
                : new HtmlString('<span>0</span> ' . Blade::render('<x-filament::icon icon="heroicon-m-lock-closed" class="h-5 w-5 text-warning-500 inline-block ml-1" />')))
            ->emptyStateDescription(fn (): string => DashboardAccess::allowsWidget(DashboardAccess::WIDGET_MEMBERSHIP_OVERVIEW_TABLE)
                ? ($this->activeTab === 'expired'
